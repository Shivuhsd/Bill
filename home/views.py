...
def Billing(request,pk):
    ...
    if request.method == 'POST':
        ...
        c_gst = ((int(amount)*int(mer.c_gst))/100)
        s_gst = ((int(amount)*int(mer.s_gst))/100)
        total_amt = (int(c_gst) + int(s_gst) + int(amount))
...
def EditBill(request, pk):
    ...
    if request.method == 'POST':
         ...
         if form.is_valid():
            ...
            rem.c_gst = ((int(rem.amount)*int(mer.c_gst))/100)
            rem.s_gst = ((int(rem.amount)*int(mer.s_gst))/100)
            rem.total_amount = (int(rem.c_gst) + int(rem.s_gst) + int(rem.amount))
            rem.save()
            return redirect('search')
...
def View_bill(request, pk):
    ...
    p_id = pay_info.patient_id.id

    p_info = Patient_info.objects.get(id=p_id)
...
def salesRegister(request):
    ...
    context = {'records':records, 'lst':lst, 'cus_info':cus_info, 'g_total':g_total, 'total':total, 'start':start_date, 'end':end_date}
...
@login_required(login_url='loginPage')
def generate_pdf(request, fdate, tdate):
    start_date = None
    end_date = None
    lst = None
    records = None
    ...
    html = render_to_string('salespfd.html', context)
    ...
    if pisa_status.err:
        return HttpResponse('PDF Conversion Failed', content_type='text/plain')
    
    response = HttpResponse(buffer.getvalue(), content_type='application/pdf')

    response['Content-Disposition'] = 'attachment; filename = "Salse Register.pdf"'
